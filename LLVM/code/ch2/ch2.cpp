...
static int gettok() // 입력값의 다음 token을 리턴
{
    static int LastChar = ' ';

    while(isspace(LastChar)) // 공백이면 입력 또는 무시
    { 
        LastChar = getchar();
    }

    if(isalpha(LastChar)) // 입력값이 알파벳인지 검사
    {
    ...
    if(isdigit(LastChar) || LastChar == '.') // 입력값이 숫자 or dot 인지 검사
    {
        std::string NumStr;
    ...
    // 그 외의 경우 (변수도 아니고 숫자도 아니고 #도 아닌) 아스키 값 리턴
...
static int GetTokPrecedence() 
{
    ...
    // 없으면 값이 0으로 나옴 그래서 아래 if에서 -1로 리턴
...
static std::unique_ptr<ExprAST> ParseParenExpr() 
{
    ...
    if(CurTok != ')') { return LogError("expedcted ')'"); }
