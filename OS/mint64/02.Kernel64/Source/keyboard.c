...
BOOL ActivateKeyboard(void)
{
    ...
    for(i=0; i < 0xFFFF; i++)
    {
        if(IsInputBufferFull() == FALSE) { break; }
    }
...
void EnableA20Gate(void)
{
    ...
    for(i=0; i<0xFFFF; i++)
    {
        if(IsOutputBufferFull() == TRUE) { break; }
    }
...
void UpdateCombinationKeyStatusAndLED(BYTE bScanCode)
{
    ...
    if(bScanCode & 0x80)   // up = down + 0x80, upCode if result is 0x80
    {
        bDown = FALSE;
        bDownScanCode = bScanCode & 0x7F;   // 0111 1111
    }
    else
    {
        bDown = TRUE;
