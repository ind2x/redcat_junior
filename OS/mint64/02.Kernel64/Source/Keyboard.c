...
BOOL kActivateKeyboard(void)
{
    ...
    kOutPortByte(0x64, 0xAE);    // 키보드 컨트롤러에 키보드 활성화 값 전송
    ...
    kOutPortByte(0x60, 0xF4);    // 키보드에 키보드 활성화 값 전송

    bResult = kWaitForACKAndPutOtherScanCode(); // ACK가 오는지 확인
...
BOOL kChangeKeyboardLED(BOOL bCapsLockOn, BOOL bNumLockOn, BOOL bScrollLockOn)
{
    ...
    kOutPortByte(0x60, 0xED);    // LED 활성화 명령 전송
    for (i = 0; i < 0xFFFF; i++)
    {
        if (kIsInputBufferFull() == FALSE)
        {
    ...
    bResult = kWaitForACKAndPutOtherScanCode();  // ACK 대기
...
void kEnableA20Gate(void)
{
    ...
    kOutPortByte(0x64, 0xD0);    // D0는 출력포트 값을 출력버퍼로 복사해오는 것
    ...
    bOutputPortData = kInPortByte(0x60); // 출력버퍼를 읽어옴
    ...
    kOutPortByte(0x64, 0xD1);    // D1은 입력버퍼의 값을 출력포트로 복사한다는 명령

    kOutPortByte(0x60, bOutputPortData); // 입력버퍼에 값을 입력하면 출력포트에 값을 복사함
}
...
void kReboot(void)
{
    ...
    kOutPortByte(0x64, 0xD1);    // 입력버퍼 값을 출력포트로 복사할 것임을 전달
    kOutPortByte(0x60, 0x00);    // 입력버퍼에 리셋 값을 써서 출력포트로 복사
...
void kUpdateCombinationKeyStatusAndLED(BYTE bScanCode)
{
    ...
    // Caps kLock 설정 후 LED 상태 변경
    else if ((bDownScanCode == 58) && (bDown == TRUE))
    {
        gs_stKeyboardManager.bCapsLockOn ^= TRUE;
        bLEDStatusChanged = TRUE;
    }
    // Num kLock 설정 후 LED 상태 변경
    else if ((bDownScanCode == 69) && (bDown == TRUE))
    {
        gs_stKeyboardManager.bNumLockOn ^= TRUE;
        bLEDStatusChanged = TRUE;
    }
    // Scroll kLock 설정 후 LED 상태 변경
...
BOOL kConvertScanCodeToASCIICode(BYTE bScanCode, BYTE *pbASCIICode, BOOL *pbFlags)
{
    ...
    if (gs_stKeyboardManager.iSkipCountForkPause > 0) // kPause 수신여부 확인
    {
        gs_stKeyboardManager.iSkipCountForkPause--;
        return FALSE;
    }

    if (bScanCode == 0xE1) // kPause 키인지 확인, 플래그 설정 후 종료
    {
        *pbASCIICode = KEY_PAUSE;
        *pbFlags = KEY_FLAGS_DOWN;
        gs_stKeyboardManager.iSkipCountForkPause = KEY_SKIPCOUNTFORPAUSE;
        return TRUE;
    }
    ...
    bUseCombinedKey = kIsUseCombinedCode(bScanCode); // 스캔코드가 조합키인지 확인
    ...
    kUpdateCombinationKeyStatusAndLED(bScanCode); // 키보드 상태 갱신
    return TRUE;
}
...
BOOL kConvertScanCodeAndPutQueue(BYTE bScanCode)
{
    ...
    if (kConvertScanCodeToASCIICode(bScanCode, &(stData.bASCIICode), &(stData.bFlags)) == TRUE) // 변환에 성공했으면 큐에 삽입
    {
