...
void DrawRect(int iX1, int iY1, int iX2, int iY2, COLOR stColor, BOOL bFill)
{
    ...
    if (bFill == FALSE)
    {
        ...
        DrawLine(iX1, iY1, iX2, iY1, stColor);
        DrawLine(iX1, iY1, iX1, iY2, stColor);
        DrawLine(iX2, iY1, iX2, iY2, stColor);
        DrawLine(iX1, iY2, iX2, iY2, stColor);
    }
    else
    {
        // VBE 모드 정보 블록을 반환
        ...
        // MemSetWord() 함수는 X의 값이 증가하는 방향으로 데이터를 채우므로
        ...
        for (iY = iY1; iY != iY2; iY += iStepY)
        {
            ...
            MemSetWord(pstVideoMemoryAddress, stColor, iWidth);
...
void DrawCircle(int iX, int iY, int iRadius, COLOR stColor, BOOL bFill)
{
    ...
    {
        // 시작점은 네 접점 모두 그림
        DrawPixel(0 + iX, iRadius + iY, stColor);
        DrawPixel(0 + iX, -iRadius + iY, stColor);
        DrawPixel(iRadius + iX, 0 + iY, stColor);
        DrawPixel(-iRadius + iX, 0 + iY, stColor);
    }
    else
    {
        // 시작 직선은 X축과 Y축 모두 그림
        DrawLine(0 + iX, iRadius + iY, 0 + iX, -iRadius + iY, stColor);
        DrawLine(iRadius + iX, 0 + iY, -iRadius + iX, 0 + iY, stColor);
    }
    ...
    for (iCircleX = 1; iCircleX <= iCircleY; iCircleX++)
    {
        ...
        {
            // 8 방향 모두 점 그림
            DrawPixel(iCircleX + iX, iCircleY + iY, stColor);
            DrawPixel(iCircleX + iX, -iCircleY + iY, stColor);
            DrawPixel(-iCircleX + iX, iCircleY + iY, stColor);
            DrawPixel(-iCircleX + iX, -iCircleY + iY, stColor);
            DrawPixel(iCircleY + iX, iCircleX + iY, stColor);
            DrawPixel(iCircleY + iX, -iCircleX + iY, stColor);
            DrawPixel(-iCircleY + iX, iCircleX + iY, stColor);
            DrawPixel(-iCircleY + iX, -iCircleX + iY, stColor);
        }
        else
        {
            ...
            DrawRect(-iCircleX + iX, iCircleY + iY,
                      iCircleX + iX, iCircleY + iY, stColor, TRUE);
            DrawRect(-iCircleX + iX, -iCircleY + iY,
                      iCircleX + iX, -iCircleY + iY, stColor, TRUE);
            DrawRect(-iCircleY + iX, iCircleX + iY,
 ...
 *  문자 출력
 */
void DrawText(int iX, int iY, COLOR stTextColor, COLOR stBackgroundColor,
