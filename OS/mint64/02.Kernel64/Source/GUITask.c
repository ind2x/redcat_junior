...
void kSystemMonitorTask(void)
{
    ...
    qwWindowID = kCreateWindow((stScreenArea.iX2 - iWindowWidth) / 2,
                               ...
                               iWindowWidth, SYSTEMMONITOR_WINDOW_HEIGHT, WINDOW_FLAGS_DEFAULT & ~WINDOW_FLAGS_SHOW, "System Monitor");
    // 윈도우를 생성하지 못했으면 실패
    if (qwWindowID == WINDOW_INVALIDID)
    {
        return;
    ...
    while (1)
    {
        ...
        for (i = 0; i < iProcessorCount; i++)
        {
            ...
            {
                // 화면에 현재 프로세서의 부하를 표시
                kDrawProcessorInformation(qwWindowID, i * SYSTEMMONITOR_PROCESSOR_WIDTH + (i + 1) * SYSTEMMONITOR_PROCESSOR_MARGIN, WINDOW_TITLEBAR_HEIGHT + 28,
                                          i);
            }
        }
                                      ...
                                      BYTE bAPICID)
{
    ...
    iMiddleX = (SYSTEMMONITOR_PROCESSOR_WIDTH -
                (kStrLen(vcBuffer) * FONT_ENGLISHWIDTH)) /
               2;
    ...
    for (j = 0; j < CONSOLE_HEIGHT; j++)
    {
        ...
        if (bChanged == TRUE)
        {
            ...
            kSetRectangleData(2, j * FONT_ENGLISHHEIGHT + WINDOW_TITLEBAR_HEIGHT,
                              5 + FONT_ENGLISHWIDTH * CONSOLE_WIDTH, (j + 1) * FONT_ENGLISHHEIGHT + WINDOW_TITLEBAR_HEIGHT - 1, &stLineArea);
            // 윈도우의 일부만 화면 업데이트
