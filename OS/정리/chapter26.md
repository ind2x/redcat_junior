...
testfileio 명령어를 실행하면 계속 에러가 발생하는데 이유를 모르겠다.. 주로 멈추는 구간은 7번 구간이거나 위에처럼 클러스터 에러.

거의 10번에 한번꼴로 되긴 하는데 코드 상에서 잘못되었다면 분명히 계속 에러가 나는게 맞는데..

같은 환경에서 같은 코드로 실행해도 되는 사람이 있고 안되는 사람이 있으며, 아예 안되는 것도 아니고 어쩌다 되기도 하는게 가상환경 문제인가 메모리 문제인가 아휴
