...
MINT 파일 시스템은 파티션을 사용하지 않으므로 파티션 영역을 0으로 초기화해주면 되겠다.
...
**디렉터리 엔트리**는 **파일의 정보를 저장하는 자료구조**로 총 32바이트이다.

파일의 이름, 크기, 시작 클러스터 인덱스를 저장하며, 루트 디렉터리는 클러스터 0을 사용한다.
...
저수준 함수에는 ```파일 시스템 초기화 함수, 파일 시스템 생성 함수, 클러스터 링크 테이블 제어 함수, 루트 디렉터리 제어 함수```가 있다.
