import sys  # sys.argv
import os   # os.scandir, os.remove
...
# Search VM files in argv[1](Directory)
VMFiles = []
with os.scandir(sys.argv[1]) as files:
    for file in files:
        # if output file exists, delete it
        if file.name == sys.argv[2]:
            os.remove(sys.argv[1]+sys.argv[2])
            continue
        if file.is_file() and file.name[-3:] == ".vm":
            if file.name == "Sys.vm":
                VMFiles.insert(0, file.name)
            else:
                VMFiles.append(file.name)
